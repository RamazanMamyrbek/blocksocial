...
class LimitStore(context: Context) {
    ...
    val limits: Flow<Map<String, Int>> = store.data.map { preferences ->
        preferences.asMap()
            .mapNotNull { (key, value) ->
                ...
                if (minutes in MINIMUM_MINUTES..MAXIMUM_MINUTES) key.name to minutes else null
            }
            .toMap()
    }
    ...
    suspend fun setLimit(app: String, minutes: Int) {
        store.edit { preferences ->
            preferences[intPreferencesKey(app)] = minutes.coerceIn(MINIMUM_MINUTES, MAXIMUM_MINUTES)
        }
    }

    suspend fun removeLimit(app: String) {
        store.edit { preferences -> preferences.remove(intPreferencesKey(app)) }
    }

    companion object {
        const val MINIMUM_MINUTES = 5
