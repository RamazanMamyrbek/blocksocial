...
This is not a bypass defect. It is a protection-availability defect, and it is the strongest argument yet for the protection health screen in phase 19: the application must be able to tell the user that blocking is not running, because the device gives no other signal. Phase 20 must decide how the service is brought back.
...
Both findings belong to the same question and are carried to phase 19.
